    ...
    div(classes = "nhsuk-error-summary") {
        ...
        div(classes = "nhsuk-error-summary__body") {
            ul(classes = "nhsuk-list nhsuk-error-summary__list") {
                errors.forEach { (field, message) -> li { a(href = "#$field") { +message } } }
            }
        }
    }
}
...
fun FlowContent.nhsTextInput(
    ...
    div(classes = "nhsuk-form-group${if (hasError) " nhsuk-form-group--error" else ""}") {
        ...
        input(
            classes =
                "nhsuk-input${if (hasError) " nhsuk-input--error" else ""}${if (extraClasses.isNotEmpty()) " $extraClasses" else ""}"
        ) {
            this.id = id
            this.name = name
            this.type =
                InputType.values().firstOrNull { it.realValue == inputType } ?: InputType.text
            this.value = value
            ...
            if (describedBy.isNotEmpty())
                attributes["aria-describedby"] = describedBy.joinToString(" ")
        }
    }
}
...
fun FlowContent.nhsDateInput(
    ...
    val hasError = error != null
    div(classes = "nhsuk-form-group${if (hasError) " nhsuk-form-group--error" else ""}") {
        // kotlinx.html doesn't expose fieldset on DIV/FlowContent, so use the tag constructor
        // directly
        FIELDSET(attributesMapOf("class", "nhsuk-fieldset"), consumer).visit {
            LEGEND(
                    attributesMapOf("class", "nhsuk-fieldset__legend nhsuk-fieldset__legend--m"),
                    consumer,
                )
                .visit { +fieldLegend }
            hint?.let {
                    ...
                    .forEach { (fieldName, fieldLabel, fieldValue) ->
                        div(classes = "nhsuk-date-input__item") {
                            div(classes = "nhsuk-form-group") {
                                ...
                                input(
                                    classes =
                                        "nhsuk-input nhsuk-date-input__input${if (hasError) " nhsuk-input--error" else ""} ${if (fieldName == "dobYear") "nhsuk-input--width-4" else "nhsuk-input--width-2"}"
                                ) {
                                    this.id = fieldName
                                    this.name = fieldName
    ...
    div(classes = "nhsuk-form-group${if (hasError) " nhsuk-form-group--error" else ""}") {
        FIELDSET(attributesMapOf("class", "nhsuk-fieldset"), consumer).visit {
            LEGEND(
                ...
                .visit { +fieldLegend }
            hint?.let { h -> div(classes = "nhsuk-hint") { +h } }
            error?.let {
                span(classes = "nhsuk-error-message") {
                    this.id = "$id-error"
                    span(classes = "nhsuk-u-visually-hidden") { +"Error:" }
