...
class MedicalRouteTest {
    ...
    private fun validMedicalParams() =
        mapOf(
            "bpSystolic" to "120",
            "bpDiastolic" to "80",
            "treatment" to "Drug",
    ...
    fun `GET medical with valid session returns form`() = testApplication {
        ...
        cookieClient.post("/person") {
            ...
            setBody(
                "nhsNumber=9434765919&firstName=Jane&lastName=Doe&email=jane.doe@example.com&dobDay=15&dobMonth=6&dobYear=1990&gender=Female"
            )
        }
    ...
    fun `GET dashboard shows chart canvas and submission count`() = testApplication {
        ...
        assertTrue(
            doc.select(".db-stat-value").isNotEmpty(),
            "Total submissions count should appear",
        )
        assertEquals("2", doc.select(".db-stat-value").first()?.text())
        // Enrol button present
        assertTrue(
            doc.select(".db-enrol-btn").isNotEmpty(),
            "Enrol New Patient button should appear",
        )
